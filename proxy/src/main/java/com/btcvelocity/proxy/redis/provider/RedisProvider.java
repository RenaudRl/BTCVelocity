...
import com.btcvelocity.proxy.redis.transaction.TransactionData;
...
public sealed interface RedisProvider permits AbstractRedisProvider {
   ...
   * {@link com.btcvelocity.proxy.redis.packet.DataPacket}.
   *
   ...
   */
  CompletableFuture<@Nullable String> getAsync(@NotNull String key);

  /**
   * Asynchronously sets a key with an expiry time in seconds.
   *
   * @param key        the key to set
   * @param value      the value to store
   * @param ttlSeconds the time-to-live in seconds
   ...
   */
  CompletableFuture<Void> setWithExpiryAsync(@NotNull String key, @NotNull String value, long ttlSeconds);
   ...
   */
  CompletableFuture<Boolean> existsKeyAsync(@NotNull String key);
   ...
   * @return a future that completes when the operation is done
   */
  CompletableFuture<Void> deleteKeyAsync(@NotNull String key);
