...
package com.btcvelocity.proxy.queue;

import com.btcvelocity.api.queue.QueueManager;
import com.btcvelocity.api.queue.QueueState;
import com.btcvelocity.api.queue.ServerStatus;
import com.btcvelocity.proxy.queue.util.QueueComponents;
...
import static com.btcvelocity.api.queue.QueueState.ACTIVE;
import static com.btcvelocity.api.queue.QueueState.PAUSED;
import static com.btcvelocity.api.queue.ServerStatus.FULL;
import static com.btcvelocity.api.queue.ServerStatus.OFFLINE;
import static com.btcvelocity.api.queue.ServerStatus.ONLINE;
import static com.btcvelocity.api.queue.ServerStatus.WAITING;
import static com.btcvelocity.proxy.permission.PermissionUtils.findHighestPermissionValue;
...
public class VelocityQueueManager implements QueueManager {
   ...
   */
  protected final Map<String, Long> lastTurnedOnlineTime = new ConcurrentHashMap<>();
  ...
  private void runTransfer() {
    ...
    final int maxBatch = server.getConfiguration().getQueue().getMaxBatchSize();
    ...
    queues.values().stream()
        ...
        .filter(q -> q.size() > 0)
        .forEach(queue -> {
          // Transfer up to maxBatchSize candidates from this queue per tick.
          final List<? extends VelocityQueueEntry> candidates = queue.getInternalEntries().stream()
          ...
          for (VelocityQueueEntry candidate : candidates) {
  ...
  private void pingBackends() {
    ...
    final List<CompletableFuture<?>> pingFutures = new ArrayList<>();
    ...
    for (VelocityQueue queue : queues.values()) {
      ...
      final CompletableFuture<?> pingFuture = rs.ping()
          ...
          .whenComplete((result, th) -> {
            ...
            if (queue.getServerStatus() == OFFLINE) {
              ...
              lastTurnedOnlineTime.put(queue.getName(), System.currentTimeMillis());
            }
            ...
            final Long lastOnline = lastTurnedOnlineTime.get(queue.getName());
            if (lastOnline != null) {
    ...
    // Wait for every backend ping to complete so all status transitions are applied
